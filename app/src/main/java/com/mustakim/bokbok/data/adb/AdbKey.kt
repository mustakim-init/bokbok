...
class AdbKey(private val adbKeyStore: AdbKeyStore, name: String) {
    ...
    init {
        this.encryptionKey = getOrCreateEncryptionKey() ?: error("Failed to generate encryption key")
    ...
    private fun getOrCreatePrivateKey(): RSAPrivateKey {
        ...
        if (privateKey == null) {
            ...
            val encrypted = encrypt(privateKey.encoded, aad)
            if (encrypted != null) {
    ...
    @delegate:RequiresApi(android.os.Build.VERSION_CODES.R)
    ...
    private val keyManager get() = object : X509ExtendedKeyManager() {
        ...
        override fun chooseClientAlias(keyTypes: Array<out String>, issuers: Array<out Principal>?, socket: Socket?): String? = if ("RSA" in keyTypes) alias else null
        override fun getCertificateChain(alias: String?): Array<X509Certificate>? = if (alias == this.alias) arrayOf(certificate) else null
    ...
    private val trustManager get() = @androidx.annotation.RequiresApi(android.os.Build.VERSION_CODES.R) object : X509ExtendedTrustManager() {
        @SuppressLint("TrustAllX509TrustManager") override fun checkClientTrusted(chain: Array<out X509Certificate>?, authType: String?, socket: Socket?) {}
        @SuppressLint("TrustAllX509TrustManager") override fun checkClientTrusted(chain: Array<out X509Certificate>?, authType: String?, engine: SSLEngine?) {}
        @SuppressLint("TrustAllX509TrustManager") override fun checkClientTrusted(chain: Array<out X509Certificate>?, authType: String?) {}
        @SuppressLint("TrustAllX509TrustManager") override fun checkServerTrusted(chain: Array<out X509Certificate>?, authType: String?, socket: Socket?) {}
        @SuppressLint("TrustAllX509TrustManager") override fun checkServerTrusted(chain: Array<out X509Certificate>?, authType: String?, engine: SSLEngine?) {}
        @SuppressLint("TrustAllX509TrustManager") override fun checkServerTrusted(chain: Array<out X509Certificate>?, authType: String?) {}
...
class SharedPreferencesAdbKeyStore(private val preference: SharedPreferences) : AdbKeyStore {
    private val preferenceKey = "adbkey"
    override fun put(bytes: ByteArray) {
        preference.edit { putString(preferenceKey, Base64.encodeToString(bytes, Base64.NO_WRAP)) }
    }
    override fun get(): ByteArray? {
        val encoded = preference.getString(preferenceKey, null) ?: return null
