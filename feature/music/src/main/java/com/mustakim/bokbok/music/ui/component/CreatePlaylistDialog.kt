...
import com.mustakim.bokbok.music.extensions.isSyncEnabled
...
fun CreatePlaylistDialog(
    ...
    TextFieldDialog(
        icon = { Icon(painter = painterResource(CoreR.drawable.playlist_add), contentDescription = null) },
        title = { Text(text = stringResource(MusicR.string.create_playlist)) },
        placeholder = { Text(text = stringResource(CoreR.string.playlist_name)) },
        ...
        extraContent = {
            if (allowSyncing) {
                ...
                val syncDescription = when {
                    !isSignedIn -> stringResource(CoreR.string.not_logged_in_youtube)
                    !isYtmSyncEnabled -> stringResource(CoreR.string.sync_disabled)
                    else -> stringResource(CoreR.string.allows_for_sync_witch_youtube)
                }
                Surface(
                    ...
                    Row(
                        ...
                        Column(
                            ...
                            Text(
                                text = stringResource(CoreR.string.sync_playlist),
                        ...
                        Switch(
                            ...
                            onCheckedChange = {
                                ...
                                if (!isSignedIn) {
                                    Toast.makeText(
                                        context,
                                        context.getString(CoreR.string.not_logged_in_youtube),
                                        Toast.LENGTH_SHORT
                                    ).show()
                                    return@Switch
                                }
                                if (!isYtmSyncEnabled) {
                                    Toast.makeText(
                                        context,
                                        context.getString(CoreR.string.sync_disabled),
                                        Toast.LENGTH_SHORT
                                    ).show()
                                    return@Switch
                                }
