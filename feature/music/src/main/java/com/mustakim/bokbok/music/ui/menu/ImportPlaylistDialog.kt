...
fun ImportPlaylistDialog(
    ...
    if (isVisible) {
        TextFieldDialog(
            icon = { Icon(painter = painterResource(CoreR.drawable.add), contentDescription = null) },
            title = { Text(text = stringResource(CoreR.string.import_playlist)) },
            ...
            onDone = { finalName ->
                ...
                coroutineScope.launch(Dispatchers.IO) {
                    try {
                        ...
                        if (ids.isEmpty()) {
                            showMessage(context.getString(CoreR.string.import_failed))
                            withContext(Dispatchers.Main) {
                                resetState()
                                onDismiss()
                            }
                        ...
                        showMessage(context.getString(CoreR.string.playlist_synced))
                        withContext(Dispatchers.Main) {
                            resetState()
                            onDismiss()
                        }
                        ...
                        e.printStackTrace()
                        showMessage(context.getString(CoreR.string.import_failed) + ": ${e.message ?: "Unknown error"}")
                        withContext(Dispatchers.Main) {
                            resetState()
                            onDismiss()
                        }
        ...
        DefaultDialog(
            ...
            content = {
                Column(horizontalAlignment = Alignment.CenterHorizontally) {
                    Text(text = stringResource(CoreR.string.already_in_playlist))
                    if (isProcessingDuplicate) {
            ...
            buttons = {
                ...
                TextButton(
                    ...
                    shapes = ButtonDefaults.shapes()
                ) { Text(text = stringResource(CoreR.string.update_button)) }
                ...
                TextButton(
                    ...
                    shapes = ButtonDefaults.shapes()
                ) { Text(text = stringResource(CoreR.string.import_playlist)) }
            }
        )
    }
}
