    ...
    private fun PoTokenExtractionContent(targetUrl: String) {
        ...
        fun triggerExtraction() {
            ...
            if (!isAtDestination(currentUrl, targetUrl)) {
                Toast.makeText(context, MusicR.string.open_account_before_extract, Toast.LENGTH_SHORT).show()
                return
            }
            ...
            webView?.postDelayed({
                ...
                if (extractedVisitorData.isNullOrBlank() || extractedGvsToken.isNullOrBlank()) {
                    isExtracting = false
                    Toast.makeText(context, MusicR.string.token_generation_failed, Toast.LENGTH_SHORT).show()
                }
            }, 4000L)
        }
        ...
        Box(modifier = Modifier.fillMaxSize()) {
            ...
            TopAppBar(
                title = { Text(stringResource(MusicR.string.extracting_from_url)) },
                navigationIcon = {
                    BokBokIconButton(
            ...
            ExtendedFloatingActionButton(
                text = {
                    Text(
                        if (isExtracting) {
                            stringResource(MusicR.string.generating_tokens)
                        } else {
                            stringResource(MusicR.string.regenerate_token)
                        }
                    )
                },
                icon = {
