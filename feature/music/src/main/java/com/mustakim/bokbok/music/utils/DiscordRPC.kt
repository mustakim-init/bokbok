...
class DiscordRPC(
    ...
    companion object {
        ...
        private const val PAUSE_IMAGE_URL =
            "https://raw.githubusercontent.com/mustakim/BokBok/main/fastlane/metadata/android/en-US/images/RPC/pause_icon.png"
        private const val APP_ICON_URL = 
            "https://raw.githubusercontent.com/mustakim/BokBok/main/fastlane/metadata/android/en-US/images/icon.png"
    ...
    suspend fun updateSong(
        ...
        val activityName = when (namePref.uppercase()) {
            "ARTIST" -> translatedMap["{artist}"] ?: pickSourceValue(namePref, song, context.getString(CoreR.string.app_name))
            "ALBUM" -> translatedMap["{album}"] ?: pickSourceValue(namePref, song, context.getString(CoreR.string.app_name))
            "SONG" -> translatedMap["{song}"] ?: pickSourceValue(namePref, song, context.getString(CoreR.string.app_name))
            "APP" -> context.getString(CoreR.string.app_name)
            else -> pickSourceValue(namePref, song, context.getString(CoreR.string.app_name))
        }
        ...
        if (activityDetails.isNullOrBlank()) {
            activityDetails = song.song.title.ifBlank { context.getString(CoreR.string.app_name) }
        }
        ...
        if (activityState.isNullOrBlank()) {
            activityState = song.artists.firstOrNull()?.name ?: context.getString(CoreR.string.app_name)
        }
        ...
        val button2Label = context.dataStore[DiscordActivityButton2LabelKey] ?: "Go to BokBok"
        ...
        val button2CustomUrl = context.dataStore[DiscordActivityButton2CustomUrlKey] ?: "https://github.com/mustakim/BokBok"
        ...
        val sendSmallText = if (isPaused) {
            context.getString(MusicR.string.discord_paused)
        } else {
    ...
    suspend fun refreshActivity(song: Song, currentPlaybackTimeMillis: Long, isPaused: Boolean = false) = runCatching {
        try {
            ...
            com.mustakim.bokbok.music.utils.GlobalLog.append(
