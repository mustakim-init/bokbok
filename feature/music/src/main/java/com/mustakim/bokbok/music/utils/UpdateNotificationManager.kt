...
object UpdateNotificationManager {
    ...
    fun createNotificationChannel(context: Context) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val name = context.getString(MusicR.string.update_notification_channel_name)
            val descriptionText = context.getString(MusicR.string.update_notification_channel_desc)
    ...
    fun checkForUpdates(context: Context) {
        scope.launch {
            try {
                ...
                val lastCheck: Long = dataStore.data.map { it[LastUpdateCheckKey] ?: 0L }.first()
    ...
    suspend fun notifyIfNewVersion(context: Context, latestVersion: String) {
        try {
            ...
            if (latestVersion != lastNotified && !Updater.isSameVersion(latestVersion, BuildConfig.VERSION_NAME)) {
                val downloadUrl = Updater.getLatestDownloadUrl(context) ?: ""
    ...
    private fun showUpdateNotification(context: Context, newVersion: String, downloadUrl: String) {
        createNotificationChannel(context)

        val openAppIntent = Intent(context, Class.forName("com.mustakim.bokbok.MainActivity")).apply {
        ...
        val notification = NotificationCompat.Builder(context, CHANNEL_ID)
            ...
            .setContentTitle(context.getString(MusicR.string.update_notification_title))
            .setContentText(context.getString(MusicR.string.update_notification_text, newVersion))
            .setPriority(NotificationCompat.PRIORITY_DEFAULT)
            .setContentIntent(openAppPendingIntent)
            .setAutoCancel(true)
            .addAction(
                CoreR.drawable.download,
                context.getString(MusicR.string.download),
