package com.mustakim.bokbok.music.kugou

import com.mustakim.bokbok.music.kugou.models.DownloadLyricsResponse
import com.mustakim.bokbok.music.kugou.models.Keyword
import com.mustakim.bokbok.music.kugou.models.SearchLyricsResponse
import com.mustakim.bokbok.music.kugou.models.SearchSongResponse
...
object KuGou {
    ...
    suspend fun getAllPossibleLyricsOptions(
        ...
        val keyword = generateKeyword(title, artist)
        for (info in searchSongs(keyword).data.info) {
            if (duration == -1 || abs(info.duration - duration) <= DURATION_TOLERANCE) {
                searchLyricsByHash(info.hash).candidates.firstOrNull()?.let { candidate ->
                    Base64.Default.decode(
                        downloadLyrics(candidate.id, candidate.accesskey).content
        ...
        for (candidate in searchLyricsByKeyword(keyword, duration).candidates) {
            Base64.Default.decode(
                downloadLyrics(candidate.id, candidate.accesskey).content
            ).decodeToString().normalize().let(callback)
        }
    ...
    suspend fun getLyricsCandidate(
        ...
        for (song in searchSongs(keyword).data.info) {
    ...
    suspend fun searchSongs(keyword: Keyword) =
        client.get("https://mobileservice.kugou.com/api/v3/search/song") {
            ...
            parameter("keyword", "${keyword.title} - ${keyword.artist}")
